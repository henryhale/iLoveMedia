...
<template>
	<div class="max-w-5xl mx-auto space-y-6">
		<div
			...
			<!-- Dropzone -->
			<div v-if="!imageUrl"
				class="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-3xl p-16 text-center hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors relative group">
				<input type="file" accept="image/*" @change="handleFileChange"
					class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
				<div class="space-y-4">
					<div
						class="mx-auto w-16 h-16 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
					...
					<div class="pt-2">
						<span
							class="text-[10px] font-bold uppercase tracking-widest px-3 py-1 bg-slate-100 dark:bg-slate-700 rounded-full text-slate-500">Fast
							Local OCR</span>
					</div>
				</div>
			</div>
			...
			<div v-else class="space-y-8">
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
					...
					<div class="space-y-3">
						<label class="text-xs font-bold uppercase tracking-widest text-slate-400">Source Image</label>
						<div
							class="aspect-video bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex items-center justify-center relative shadow-inner">
							<img :src="imageUrl" alt="Source" class="max-w-full max-h-full object-contain" />

							<div v-if="isProcessing"
								class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center text-white p-6 text-center">
								...
								<div class="w-full max-w-xs h-1.5 bg-white/20 rounded-full mt-4 overflow-hidden">
					...
					<div class="space-y-3 flex flex-col">
						<div class="flex justify-between items-center">
							<label class="text-xs font-bold uppercase tracking-widest text-slate-400">Extracted
								Text</label>
						...
						<div
							class="grow min-h-75 lg:min-h-0 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl relative overflow-hidden group shadow-inner">
							<textarea v-model="extractedText" :placeholder="isProcessing
								? 'Analyzing image...'
								: 'Extracted text will appear here...'
								" class="w-full h-full p-6 bg-transparent text-slate-800 dark:text-slate-200 font-mono text-sm resize-none focus:outline-none placeholder-slate-400 scrollbar-thin" />
							<div v-if="!extractedText && !isProcessing"
				...
				<!-- Action Buttons -->
				<div class="flex flex-col sm:flex-row gap-4 pt-4 border-t dark:border-slate-700">
					<button v-if="!extractedText" @click="handleOcr" :disabled="isProcessing"
						class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-500/20 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
					...
					<button @click="reset" :disabled="isProcessing"
						class="px-8 py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50">
			...
			<div class="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
				<div
					class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
					<p class="text-xs font-bold text-slate-900 dark:text-white mb-1 uppercase tracking-wider">
						Zero Uploads
					</p>
