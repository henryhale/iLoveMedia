...
<template>
	<div class="max-w-3xl mx-auto space-y-6">
		<div
			class="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors text-center">
			<h2
				class="text-xl font-semibold mb-6 flex items-center justify-center gap-2 text-slate-900 dark:text-white">
			...
			<div class="flex flex-col items-center space-y-8">
				...
				<div class="relative w-48 h-48 rounded-full flex items-center justify-center border-4 transition-all duration-300"
					:class="isRecording
						...
						">
					<div class="text-3xl font-mono font-bold text-slate-900 dark:text-white">
						{{ formatTime(recordingTime) }}
					</div>
					<div v-if="isRecording"
						class="absolute inset-0 rounded-full border-4 border-red-500 animate-ping opacity-25"></div>
				</div>
				...
				<div class="flex gap-4">
					...
					<button v-else @click="stopRecording"
						class="bg-slate-900 dark:bg-slate-100 dark:text-slate-900 text-white px-8 py-3 rounded-full font-bold flex items-center gap-2 transition-all transform hover:scale-105 active:scale-95 shadow-md">
				...
				<div v-if="audioURL && !isRecording" class="w-full pt-8 animate-in fade-in slide-in-from-top-4">
					<div
						class="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
						...
						<div class="flex gap-2">
							<a :href="audioURL" download="recording.webm"
								class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center justify-center gap-2">
