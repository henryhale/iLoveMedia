...
const handleToggleListening = () => {
	if (engine.value === "native") {
		isListening.value ? stopNativeListening() : startNativeListening()
	} else {
		isListening.value ? stopWhisperRecording() : startWhisperRecording()
	}
}
	...
	<div class="max-w-5xl mx-auto space-y-6">
		...
		>
				...
				<div class="flex items-center gap-4">
					<div class="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-xl">
						...
						>
							Whisper AI
							<span
								class="text-[9px] bg-green-100 dark:bg-green-900 text-green-600 px-1.5 py-0.5 rounded uppercase font-bold"
								>WASM</span
							>
						</button>
					</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
				...
				<div
					class="lg:col-span-4 flex flex-col items-center gap-8 border-r dark:border-gray-700 pr-0 lg:pr-8"
				>
					<div class="relative flex flex-col items-center">
						...
						>
							<div
								v-for="i in 5"
								:key="i"
								class="w-1.5 h-8 bg-green-400/30 rounded-full animate-bounce"
					...
					<div v-if="isModelLoading" class="w-full space-y-2">
						<div
							class="flex justify-between text-[10px] font-bold text-green-600 uppercase"
						>
							<span>Loading Tiny-Whisper</span>
							<span>{{ loadProgress }}%</span>
						</div>
						<div
							class="w-full bg-gray-100 dark:bg-gray-900 h-1.5 rounded-full overflow-hidden"
						>
							<div
				...
				<div class="lg:col-span-8 space-y-4">
					...
					>
						<div
							...
							class="text-xl leading-relaxed text-gray-800 dark:text-gray-200"
						>
							<p>{{ transcription }}</p>
							<p
								v-if="interimResult"
								class="text-green-500 dark:text-green-400 font-medium transition-all animate-pulse mt-2"
							>
								{{ interimResult }}
							</p>
						</div>
						...
						<div
							v-else
							class="absolute inset-0 flex flex-col items-center justify-center text-gray-400 opacity-50 space-y-4"
						>
					...
					<!-- Footer Engine Info -->
					<div
						class="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800"
					>
						<div class="p-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
							<CheckIcon class="w-4 h-4 text-emerald-500" />
						</div>
						<div
							class="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed uppercase tracking-wide"
						>
